...
const FAQs = () => {
  ...
  const filteredFAQs = selectedCategory === 'all' 
    ? (allFAQs.length > 0 ? allFAQs : fallbackFAQs)
    : (allFAQs.length > 0 ? allFAQs : fallbackFAQs).filter(faq => faq.category === selectedCategory);

  const displayFAQs = allFAQs.length > 0 ? allFAQs : fallbackFAQs;
  const categories = [
    { id: 'all' as FAQCategory, label: 'All FAQs', icon: HelpCircle, count: displayFAQs.length },
    { id: 'getting-started' as FAQCategory, label: 'Getting Started', icon: Rocket, count: displayFAQs.filter(f => f.category === 'getting-started').length },
    { id: 'education' as FAQCategory, label: 'Education', icon: GraduationCap, count: displayFAQs.filter(f => f.category === 'education').length },
    { id: 'jobs' as FAQCategory, label: 'Jobs', icon: Briefcase, count: displayFAQs.filter(f => f.category === 'jobs').length },
    { id: 'directories' as FAQCategory, label: 'Directories', icon: FolderOpen, count: displayFAQs.filter(f => f.category === 'directories').length },
    { id: 'hands-skills' as FAQCategory, label: 'Hands & Skills', icon: Wrench, count: displayFAQs.filter(f => f.category === 'hands-skills').length }
  ];
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-white" style={{ '--tw-ring-color': 'transparent', '--tw-ring-offset-color': 'transparent' } as React.CSSProperties}>
        ...
        {loading ? (
          <div className="text-center py-5" style={{ fontFamily: fontFamily }}>Loading FAQs...</div>
        ) : (
