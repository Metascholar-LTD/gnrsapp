...
const GraduateProgramDetail = () => {
  ...
  const [program, setProgram] = useState<GraduateProgram | null>(null);
    ...
    const loadProgram = async () => {
      if (id) {
        ...
        try {
          ...
          if (error) {
            console.error("Error loading graduate program:", error);
            setLoading(false);
            return;
          }
  ...
  const goBack = () => navigate("/jobs/graduate-recruitment");
  ...
  const handleApply = () => {
    if (!program?.application_url) return;
  ...
  const heroImageUrl = program?.image_url || "https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=1200&auto=format&fit=crop&q=60";
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-2xl mx-auto bg-white border border-slate-200 rounded-2xl shadow-sm p-8 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <h1 className="text-2xl font-bold text-slate-900">Graduate Program Not Found</h1>
                <p className="text-slate-600">We couldn't find the graduate program you're looking for.</p>
            ...
            {fieldOperationGroups.length > 0 ? (
              <>
                <p className="text-slate-600">
                  Day-to-day operations and responsibilities in this graduate program.
                </p>
        ...
        return (
          <section className="space-y-6">
            ...
            <div className="rounded-2xl bg-sky-50 border border-sky-100 p-5 space-y-4">
              <div>
                <h3 className="text-lg font-semibold text-slate-900">Get notified for similar programs</h3>
                <p className="text-sm text-slate-600">Sign up to receive graduate program alerts tailored to your interests.</p>
              </div>
            </div>
          </section>
        );
          ...
          <div className="max-w-4xl">
            ...
            >
              ...
              <div className="flex items-center gap-6 mb-4 flex-wrap">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <Button
                    ...
                    onClick={handleApply}
                    disabled={!program?.application_url}
                    title={program?.application_url ? 'Apply for this program' : 'Application link not available'}
                  >
                    Apply Now
              ...
              <div 
                ...
                onClick={() => navigate(`/companies/${encodeURIComponent(program.company)}`)}
              >
                <Building2 className="w-5 h-5" />
                <span className="font-semibold hover:underline">{program.company}</span>
              </div>
              ...
              <div className="flex flex-wrap gap-4 mb-6">
                <div className="flex items-center gap-2 text-white">
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                    ...
                    <span className="font-semibold text-lg">{program.location.split(",")[0]}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-white">
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                ...
                <div className="flex items-center gap-2 text-white">
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                    <DollarSign className="w-5 h-5 inline mr-2 text-sky-400" />
                  ...
                  <div className="space-y-4">
                    <div className="group flex items-start gap-3 p-3 rounded-xl bg-white/80 hover:bg-white border border-slate-100 hover:border-sky-200/60 hover:shadow-md transition-all duration-300 ease-out">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-semibold text-slate-900">{program.type}</p>
                      </div>
                    </div>
                    ...
                    <div className="group flex items-start gap-3 p-3 rounded-xl bg-white/80 hover:bg-white border border-slate-100 hover:border-sky-200/60 hover:shadow-md transition-all duration-300 ease-out">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-0.5">Salary</p>
                        <p className="text-sm font-semibold text-slate-900">{program.salary}</p>
                      </div>
                    </div>
                    ...
                    <div className="group flex items-start gap-3 p-3 rounded-xl bg-white/80 hover:bg-white border border-slate-100 hover:border-sky-200/60 hover:shadow-md transition-all duration-300 ease-out">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-semibold text-slate-900">{program.location.split(",")[0]}</p>
                      </div>
                    </div>
                    ...
                    {program.posted && (
                      <div className="group flex items-start gap-3 p-3 rounded-xl bg-white/80 hover:bg-white border border-slate-100 hover:border-sky-200/60 hover:shadow-md transition-all duration-300 ease-out">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm font-semibold text-slate-900">{program.posted}</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-slate-600">
                      Launch your career with this structured graduate program designed for fresh graduates.
                    </p>
                    <Button 
