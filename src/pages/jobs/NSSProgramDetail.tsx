...
const NSSProgramDetail = () => {
    ...
    const loadProgram = async () => {
      if (id) {
        ...
        try {
          ...
          if (error) {
            console.error("Error loading NSS program:", error);
            setLoading(false);
            return;
          }
  ...
  const goBack = () => navigate("/jobs/nss");
  ...
  const handleApply = () => {
    if (program?.application_url) {
      let url = program.application_url.trim();
      if (!url.match(/^https?:\/\//i)) {
        url = `https://${url}`;
      }
      window.open(url, '_blank', 'noopener,noreferrer');
    } else {
      window.open("https://www.nss.gov.gh", '_blank', 'noopener,noreferrer');
    }
  };

  const heroImageUrl = program?.image_url || "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=1200&auto=format&fit=crop&q=60";
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-2xl mx-auto bg-white border border-slate-200 rounded-2xl shadow-sm p-8 text-center">
              <div className="flex flex-col items-center gap-4">
                ...
                <h1 className="text-2xl font-bold text-slate-900">NSS Program Not Found</h1>
                <p className="text-slate-600">We couldn't find the NSS program you're looking for.</p>
                <Button onClick={goBack} className="bg-slate-900 hover:bg-slate-800 text-white px-6">
                  Back to NSS Programs
                </Button>
              </div>
            </div>
          </div>
  ...
  const navItems = [
    ...
    { id: "benefits", label: "Benefits & Apply" },
  ];
  ...
  const descriptionParagraphs = program?.description_paragraphs && program.description_paragraphs.length > 0
    ? program.description_paragraphs
    : [program?.description || ""];

  const impactParagraphs = program?.impact_paragraphs || [];
  const impactHighlights = program?.impact_highlights || [];
  const fieldOperationGroups = program?.field_ops_groups || [];
  const cultureParagraphs = program?.culture_paragraphs || [];
  const opportunityParagraphs = program?.opportunity_paragraphs || [];

  const skillsData = [
    {
      title: "Formal Qualifications",
      items: program?.skills_formal_qualifications || [],
    },
    {
      title: "Additional Knowledge",
      items: program?.skills_additional_knowledge || [],
    },
    {
      title: "Experience",
      items: program?.skills_experience || [],
    },
    {
      title: "Technical Skills",
      items: program?.skills_technical || [],
    },
  ];

  const behavioralAttributes = program?.behavioral_attributes || [];
  ...
  const renderTabContent = () => {
    switch (activeNav) {
            ...
            {impactHighlights.length > 0 && (
              <div className="rounded-2xl bg-sky-50 border border-sky-100 p-5">
                <p className="text-sm font-semibold text-sky-900 uppercase tracking-wide mb-3">
                  As a service personnel you will
                </p>
            ...
            {fieldOperationGroups.length > 0 ? (
              <>
                <p className="text-slate-600">
                  Day-to-day responsibilities and operational activities during your service year.
                </p>
      ...
      case "skills":
        ...
        const hasKeySkills = program.skills && program.skills.length > 0;
          ...
          <section className="space-y-6">
            {hasBenefits && (
              <div className="rounded-2xl bg-emerald-50 border border-emerald-100 p-5">
                <h3 className="text-base font-semibold text-emerald-900 mb-2">Benefits</h3>
                {renderList(benefits)}
              </div>
            )}
            {hasCultureContent && (
            ...
            {!hasCultureContent && !hasOpportunityContent && !hasBenefits && (
              <p className="text-slate-500 italic">No additional information available yet.</p>
            )}
            <div className="rounded-2xl bg-sky-50 border border-sky-100 p-5 space-y-4">
              <div>
                <h3 className="text-lg font-semibold text-slate-900">Ready to Begin Your National Service?</h3>
                <p className="text-sm text-slate-600">Apply through the official NSS portal or use the program-specific application link.</p>
              </div>
            </div>
          </section>
        );
            ...
            >
              ...
              <div className="flex items-center gap-6 mb-4 flex-wrap">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <Button
                    size="lg"
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-lg px-8 py-6 shadow-xl"
                    onClick={handleApply}
                  >
                    Apply Now
                  ...
                  <Button
                    ...
                    title={isBookmarked ? "Remove from saved" : "Save program"}
                  >
                    {isBookmarked ? (
              ...
              <div className="flex items-center gap-2 text-emerald-400 text-xl mb-6">
                <Shield className="w-5 h-5" />
                <span className="font-semibold">National Service Scheme</span>
              </div>

              <div className="flex flex-wrap gap-4 mb-6">
                <div className="flex items-center gap-2 text-white">
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                    <Clock className="w-5 h-5 inline mr-2 text-emerald-400" />
                    <span className="font-semibold">{program.duration}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-white">
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                    <MapPin className="w-5 h-5 inline mr-2 text-emerald-400" />
                    <span className="font-semibold">{program.locations.length} location{program.locations.length > 1 ? 's' : ''}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-white">
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                    <Users className="w-5 h-5 inline mr-2 text-emerald-400" />
                    <span className="font-semibold">National Service</span>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          <section className="bg-white rounded-2xl shadow-lg p-6 md:p-8 mt-8 relative z-10">
            <div className="mt-10 grid grid-cols-1 lg:grid-cols-[2.5fr_1fr] gap-8">
              <div className="space-y-6">
                <nav className="bg-white border border-emerald-100 rounded-2xl p-2 shadow-sm">
                  <div className="flex flex-wrap gap-2">
                    {navItems.map((item) => (
                      <button
                        key={item.id}
              ...
              <aside className="space-y-4">
                <motion.div
                  ...
                  className="rounded-2xl p-6 bg-gradient-to-br from-white via-slate-50/50 to-emerald-50/30 border border-slate-200/60 shadow-lg shadow-slate-200/50 backdrop-blur-sm"
                >
                  <div className="flex items-center gap-2 mb-6">
                    <h3 className="text-lg font-bold text-slate-900 tracking-tight">Program Snapshot</h3>
                  </div>
                  <div className="space-y-4">
                    <div className="group flex items-start gap-3 p-3 rounded-xl bg-white/80 hover:bg-white border border-slate-100 hover:border-emerald-200/60 hover:shadow-md transition-all duration-300 ease-out">
                    ...
                    <div className="group flex items-start gap-3 p-3 rounded-xl bg-white/80 hover:bg-white border border-slate-100 hover:border-emerald-200/60 hover:shadow-md transition-all duration-300 ease-out">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-0.5">Locations</p>
                        <p className="text-sm font-semibold text-slate-900">{program.locations.length} location{program.locations.length > 1 ? 's' : ''}</p>
                        {program.locations.length > 0 && (
                          <p className="text-xs text-slate-500 mt-1">{program.locations.slice(0, 2).join(", ")}{program.locations.length > 2 ? "..." : ""}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="group flex items-start gap-3 p-3 rounded-xl bg-white/80 hover:bg-white border border-slate-100 hover:border-emerald-200/60 hover:shadow-md transition-all duration-300 ease-out">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-0.5">Service Type</p>
                        <p className="text-sm font-semibold text-slate-900">National Service</p>
                      </div>
                    </div>
                    ...
                    {requirements && requirements.length > 0 && (
                      <div className="group flex items-start gap-3 p-3 rounded-xl bg-white/80 hover:bg-white border border-slate-100 hover:border-emerald-200/60 hover:shadow-md transition-all duration-300 ease-out">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-0.5">Requirements</p>
                          <p className="text-sm font-semibold text-slate-900">{requirements.length} requirement{requirements.length > 1 ? 's' : ''}</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-slate-600">
                      Complete your mandatory national service with confidence. Apply through the official NSS portal.
                    </p>
                    <Button 
...
export default NSSProgramDetail;
